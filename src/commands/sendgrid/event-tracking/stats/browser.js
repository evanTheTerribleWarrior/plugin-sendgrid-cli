...
class StatsBrowserList extends BaseCommand {
    async run() {
      await super.run();
      const result = await this.listBrowserStats()
      this.output(result)
    }

    async listBrowserStats() {

        const { headers, ...data } = extractFlags(this.flags);

        const request = {
            url: `${API_PATHS.STATS_BROWSER}`,
            method: 'GET',
            qs: data,
            headers: headers
        }
...
StatsBrowserList.description = 'Retrieve your email statistics segmented by recipient browser'
StatsBrowserList.flags = Object.assign(
  { 
    'browsers': flags.string({description: 'The browsers to get statistics for. You can include up to 10 different browsers by including this parameter multiple times', required: false}),
...
module.exports = StatsBrowserList;