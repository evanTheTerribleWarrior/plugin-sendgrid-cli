...
class StatsDeviceList extends BaseCommand {
    async run() {
      await super.run();
      const result = await this.listDeviceStats()
      this.output(result)
    }

    async listDeviceStats() {

        const { headers, ...data } = extractFlags(this.flags);

        const request = {
            url: `${API_PATHS.STATS_DEVICES}`,
            method: 'GET',
            qs: data,
            headers: headers
        }
...
StatsDeviceList.description = 'Retrieve your email statistics segmented by the device type'
StatsDeviceList.flags = Object.assign(
  { 
...
module.exports = StatsDeviceList;